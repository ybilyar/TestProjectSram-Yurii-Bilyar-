    ...
    class NearBy {
        ...
        class func initWithData(_ data:[String:Any]) -> NearBy {
            ...
            if let geometry = data["geometry"] as? type, let coordinate = geometry["location"] as? type {
                ...
                let lat = savedLoc.lat ?? 0.0
                let long = savedLoc.long ?? 0.0
                let selectedLoc = CLLocation(latitude: lat, longitude: long)
                let currentLoc = CLLocation(latitude: location.latitude ?? 0.0, longitude: location.longitude ?? 0.0)
                let distance = currentLoc.distance(from: selectedLoc) / 1000
                object.distance = CGFloat(distance)
            }
            
            if let photos = data["photos"] as? [type], let first = photos.first {
                if let photoRef = first["photo_reference"] as? String {
                    object.photoReference = photoRef
                }
            }
            return object
        }
